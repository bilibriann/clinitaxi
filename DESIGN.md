...
CLINITAXi's visual language is built around a single organizing truth: this is a physical, certified instrument — solid, precise, and officially stamped. The system does not perform modernity; it performs authority. Every surface is either the deep night navy of the device body or the near-white of a clean technical document. The amber arrives as a mark of certification — the color of the seal that makes something official — and it is used sparingly because its rarity is its power.

The design is official and precise, like a well-designed government form: structure is always visible, actions are never ambiguous, and ornament earns its presence only when it carries information. The system leans on weight and position for emphasis rather than color; when amber appears, it means something. When navy dominates, it means reliability. There is no pastiche modernism here — only the confidence of 25+ years of a company that knows exactly what it is.
...
- Amber as the certification signal — sparingly applied, always meaningful
...
- Wave dividers in the hero are the only decorative element — all else is functional
- Glassmorphism in the fixed nav is the only depth trick; everything else is flat or tonal

## Colors

Deep State authority meets the amber approval stamp — a palette that reads as official before it reads as beautiful.
...
### Secondary
- **Cian Técnico** (`#25c0f4`): Cool cyan used in primary-container and gradient accents. Reserved for secondary visual interest — never as a CTA or brand-carrying color.
- **Cian Claro** (`#3dc7f5`): Lighter cyan, used as primary-container reference; appears in gradient transitions.
...
**La Regla del Ámbar.** Amber (`#fabd2e`) appears only on: primary CTAs, the accent stripe of the credential card, certification icons on dark surfaces, and the Header background (where it IS the brand). On light surfaces, amber exists only in CTA buttons. Amber never backgrounds a content section; it certifies, it does not decorate.
...
- **Nav glass** (`backdrop-filter: blur(12px)` + `bg-accent/95`): The fixed header only. Glassmorphism here is functional — the nav overlays content as the user scrolls and must remain legible without a hard edge.
...
- **Wave dividers**: The single organic shape in the system. SVG wave at the bottom of the Hero section transitions from dark to light surface. This is the page's only departure from geometric shapes and should remain isolated to the hero.
...
- **Primary (amber CTA):** Amber background (`#fabd2e`) · Dark navy text (`#0b1528`) · 16px top/bottom, 32px left/right padding · 700 weight · `hover:brightness-110` · `active:scale-95`
- **Ghost (white outline on dark):** 2px `rgba(255,255,255,0.3)` border · White text · 14px top/bottom, 32px left/right · `hover:bg-white/10`
- **Nav CTA (navy on amber header):** Primary navy background (`#0e192f`) · White text · 10px/20px padding · 600 weight · `hover:bg-secondary`

### Cards / Containers

- **Light surface cards:** `bg-surface-alt` (`#f2f4f8`) background · 16px radius · No border · `py-8 px-8` padding
- **Dark accent card (credential):** `bg-white/5` with `border border-white/10` · 16px radius · Amber 4px top stripe · `shadow-2xl shadow-black/40` · Internal padding 32px
...
- **Fixed header:** 80px height · Amber background (`#fabd2e` at 95% opacity) · Glassmorphism (`backdrop-blur-12px`) · Bottom border `border-black/10`
- **Logo:** Local taxi icon (Material Symbols) + "CLINITAXi" in 700 weight 20px Space Grotesk · `text-on-accent` (dark navy)
- **Nav links:** `text-on-accent/70` at rest · `text-on-accent` on hover · 14px 500 weight Inter
- **Mobile:** Nav links hidden; amber call icon button (44×44px) + Contáctanos CTA only

### Trust Bar

- **Style:** Dark surface (`#0e1625`) · 2-column mobile / 4-column desktop grid · 1px `rgba(255,255,255,0.1)` dividers between cells
- **Cell:** Material Symbol icon in amber + 14px 500 weight white/90 label · 20px padding mobile, 24px desktop
...
- **Structure:** 16px radius container · 4px amber top stripe · `bg-white/5 border border-white/10` background · 32px internal padding
- **Content:** 48px × 48px amber-tinted icon container (amber 20% bg + 30% border) · Extrabold white heading · Horizontal white/10 divider · Icon + text list items in white/80 · Brand footer in white/50
...
- **Do** treat amber (`#fabd2e`) as the certification signal — reserve it for primary CTAs, credential icons, and the Header background. Its scarcity is its power.
- **Do** apply Space Grotesk at 700–800 weight for all headings. The geometric construction is the brand's typographic character; never swap it for a softer display face.
- **Do** make the phone number a `<a href="tel:+56950646818">` link everywhere it appears. The primary user is on mobile.
- **Do** keep the amber top stripe on the credential card — it is the visual "stamp" that makes the card read as certified.
...
- **Don't** use the amber accent color as a section background (other than the fixed Header, which is a committed brand decision). On light pages, amber means CTA — not surface.
